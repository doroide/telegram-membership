...
async def cancel_admin_state(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text(
        "🛠 <b>Admin Panel</b>\n\nSelect an action:",
        reply_markup=_main_keyboard(),
...
async def view_statistics(callback: CallbackQuery):
    async with async_session() as session:
        ...
        lifetime_members = (await session.execute(
            select(func.count(User.id)).where(User.is_lifetime_member == True)
        )).scalar()
        tier4_users = (await session.execute(
            select(func.count(User.id)).where(User.current_tier == 4)
        )).scalar()
...
# =====================================================
# ADD USER / ADD CHANNEL / SEARCH USER (REDIRECTS)
# =====================================================
...
# =====================================================
# SHARED HELPER — DO SEND LINKS
# =====================================================

async def _do_send_links(message: Message, target_telegram_id: int):
...
# =====================================================
# SHARED HELPER — DO USER INFO
# =====================================================

async def _do_user_info(message: Message, target_id: int):
    async with async_session() as session:
        user_result = await session.execute(
        ...
        if active_memberships:
            keyboard.append([
                InlineKeyboardButton(
                    text="🔗 Send ALL Links",
                    callback_data=f"send_all_links:{user.telegram_id}"
                )
            ])
...
async def send_access_links(callback: CallbackQuery, state: FSMContext):
    await state.set_state(AdminStates.waiting_send_links_id)
    await callback.message.edit_text(
        "🔗 <b>Send Access Links</b>\n\n"
        "Send me the <b>Telegram ID</b> of the user:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
@router.message(AdminStates.waiting_send_links_id)
async def handle_send_links_id(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    if not message.text or not message.text.strip().isdigit():
        await message.answer("❌ Invalid ID. Please send a valid Telegram ID.")
        return
    await state.clear()
    await _do_send_links(message, int(message.text.strip()))
...
async def send_links_command(message: Message):
    ...
    if len(telegram_ids) == 1:
        await _do_send_links(message, telegram_ids[0])
    else:
        from backend.bot.bot import bot
        status_lines = []
...
async def user_info_prompt(callback: CallbackQuery, state: FSMContext):
...
async def handle_user_info_id(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    if not message.text or not message.text.strip().isdigit():
        await message.answer("❌ Invalid ID. Please send a valid Telegram ID.")
        return
    await state.clear()
    await _do_user_info(message, int(message.text.strip()))
...
async def user_info_command(message: Message):
    ...
    if len(parts) != 2 or not parts[1].isdigit():
        ...
        return
    await _do_user_info(message, int(parts[1]))


# =====================================================
# SEND ALL LINKS (button in userinfo)
# =====================================================

@router.callback_query(F.data.startswith("send_all_links:"))
async def send_all_links(callback: CallbackQuery):
    target_telegram_id = int(callback.data.split(":")[1])
    from types import SimpleNamespace
    fake_msg = SimpleNamespace(answer=callback.message.answer, from_user=callback.from_user)
    await _do_send_links(fake_msg, target_telegram_id)
    await callback.answer("✅ All links sent!")


# =====================================================
# SEND ONE LINK (button in userinfo)
# =====================================================
...
async def handle_csv_import(message: Message):
    ...
    if failed_csv_rows:
        ...
        await message.answer_document(
            document=BufferedInputFile(failed_content.encode("utf-8"), filename="failed_users.csv"),
